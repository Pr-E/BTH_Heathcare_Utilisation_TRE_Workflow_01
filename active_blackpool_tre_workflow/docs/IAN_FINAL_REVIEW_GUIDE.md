...
## What is new in this final-review version

### 1. Cleaning is now explicitly auditable
...
- configured missingness classification (`CRITICAL`, `STRUCTURAL_EXPECTED`, `CONDITIONAL_EXPECTED`, `UNCLASSIFIED_REVIEW`);
...
### 2. Every analysis stage prints its key findings
...
- Stage 03: episode-to-spell collapse and identifier re-verification;
- Stage 04: linkage/source overlap by pathway family;
...
- Stage 11: internal patient-level file flags and aggregate small-cell review flags.

### 3. Each stage tells the analyst what to run next
...
### 4. Aggregate stage summaries are written automatically
...
Real-data geography/source support is recalculated. Synthetic exclusions are not hard-coded.
...
K=4 is a development-informed candidate, not a forced real-data truth. K=2–6 diagnostics are recomputed and K=4 must satisfy minimum size/stability requirements.

### Disclosure gate

Stage 11 is a **pre-screen only**. Formal local TRE disclosure-control approval remains mandatory before any output leaves the secure environment.
...
- [ ] Formal disclosure-control process is understood and separate from Stage 11.
