...
def fit_propensity(
    ...
    # Empirical common support: compare only the range of propensity scores
    # observed in both groups.  This follows the earlier structural positivity
    # screen on categorical strata (for example geography).
    ...
    # Missingness is recorded as design QA because real TRE extracts can differ
    # materially from the synthetic development data.  Imputation is performed
    # only inside the propensity design matrix; raw patient fields are untouched.
    missingness_rows = []
