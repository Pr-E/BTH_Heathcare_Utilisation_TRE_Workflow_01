...
Keeping paths in YAML rather than Python is a reproducibility feature: the same
code can be ingressed into a different approved TRE workspace without editing
analysis logic.  This module also prevents accidental execution against the
...
def source_dir(config: dict[str, Any]) -> Path:
    """Return the approved real-TRE source directory and reject synthetic mode."""
    mode = str(config.get("data_source", {}).get("mode", "tre")).strip().lower()
    if mode != "tre":
        raise ValueError(
            f"TRE workflow requires data_source.mode='tre'; received {mode!r}. "
            "Use the separate development repository for synthetic runs."
        )
